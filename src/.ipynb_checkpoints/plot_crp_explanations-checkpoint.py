...
def plot_one_image_explanation_optimized(model_name, model, img, dataset, class_id, layer, prediction_num, mode,
    ...
    if "deeplab" in model_name or "unet" in model_name or "pidnet" in model_name:
        ...
        x = img.detach().requires_grad_(True)
        ...
        if "pidnet" in model_name:
            ...
            mask = resized_mask.bool().squeeze(0).squeeze(0)
    ...
    # === Reference images — load from cache, compute on GPU if missing ===
    ...
    ref_imgs_save_path = os.path.join(os.path.dirname(output_dir), "ref_imgs", f"ref_imgs_{n_refimgs}")

    attribution_ref = ATTRIBUTORS[model_name](model)
    fv_ref = VISUALIZATIONS[model_name](
        attribution_ref, dataset, get_layer_names(model, [torch.nn.Conv2d]),
        preprocess_fn=lambda x: x, path=output_dir, max_target="max", device=str(device)
    )
    ref_imgs = get_ref_images(
        fv_ref, topk_ind.tolist(), layer,
        composite=composite,
        class_id=class_id,
        n_ref=n_refimgs,
    ...
    logger.debug(f"Time to load/compute reference images: {time.time() - ref_images_start_ts:.2f}s")
