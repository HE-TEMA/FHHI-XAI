...
# Class Detector, function predict: tensor.detach.cpu.numpy 
...
def process_image(img_src, img_size, stride):
    ...
    try:
            ...
            img_src = cv2.resize(img_src, (640, 320))                   # the resize we want. (CUSTOM LINE)
...
# I have modify this class initial parameters, including conf_thresh and iou_thres
...
class Detector(DetectBackend):
    def __init__(self,
                 ...
                 device,
                 img_size=640,
                 conf_thres=0.03,               # modify this to change the initial conf thresh in nms
                 iou_thres=0.45,                # this as well  (initial values: conf_thresh= 0.25,iou_thresh= 0.45)
                 max_det=1000):
    ...
    def forward(self, x, src_shape):
        ...
        classes = [0, 2, 7]  # classes -> list[int]
        ...
        # print(x.shape)          ## custom line for printing
        return prediction

    def predict(self, img_src):
        img, img_src = process_image(img_src, self.img_size, 32)
