...
class _PersonCarDataset:
    # The annotation order intentionally differs from the KAHY detector order.
    class_names = ("person", "car")


def test_kahy_detector_class_order_overrides_dataset_order():
    names = resolve_display_class_names("yolov6s6", _PersonCarDataset())
    assert names == ("car", "person")
    assert class_name(names, 0) == "car"
    assert class_name(names, 1) == "person"


def test_prototype_box_is_selected_from_the_requested_class():
    # Detection zero is a person; detection one is a car.
...
def test_missing_live_detection_uses_same_class_stored_prototype():
    scores = torch.tensor([[0.05, 0.95]])  # live detection is person
